...
fn remote_relay_profile_generates_codex123_provider_and_null_auth_api_key() {
    ...
    assert!(
        profile
            .config_contents
            .contains(r#"model_provider = "codex123""#)
    );
    assert!(
        profile
            .config_contents
            .contains("[model_providers.codex123]")
    );
    assert!(
        profile
            .config_contents
...
"#
    ...
    let error =
        apply_relay_profile_to_home_with_switch_rules(temp.path(), &profile, "").unwrap_err();
...
#[test]
fn apply_pure_api_config_switches_auth_json_without_openai_login_requirement() {
    let temp = tempfile::tempdir().unwrap();
    std::fs::write(
        temp.path().join("auth.json"),
