...
fn watcher_install_plan_registers_rust_launcher_at_logon() {
    let plan = build_watcher_install_plan("C:/Tools/codex123.exe".into(), 9333);

    assert_eq!(plan.run_value_name, "codex123Watcher");
    assert_eq!(
        plan.run_value,
        "\"C:/Tools/codex123.exe\" --debug-port 9333"
    );
    assert_eq!(plan.shortcut_name, "codex123Watcher.lnk");
    assert_eq!(plan.shortcut_target, "C:/Tools/codex123.exe");
    assert_eq!(plan.shortcut_arguments, "--debug-port 9333");
}

#[test]
fn spawn_launcher_command_points_to_silent_binary_only() {
    let command = build_spawn_launcher_command("C:/Tools/codex123.exe", 9444);

    assert_eq!(command[0], "C:/Tools/codex123.exe");
